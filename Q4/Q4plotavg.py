...
with open('../insertion_sort/csv files/time_data_is_ran.csv', 'r') as f:
...
with open('../merge_sort/csv files/time_data_ms_ran.csv', 'r') as f:
    reader = csv.reader(f, delimiter='\t') # as headers in a .csv file are separated by tabs
    next(reader) # skip the header row
    for row in reader:
        times_ms.append(float(row[1]))

with open('../heap_sort/csv files/time_data_hs_ran.csv', 'r') as f:
    reader = csv.reader(f, delimiter='\t') # as headers in a .csv file are separated by tabs
    next(reader) # skip the header row
    for row in reader:
        times_hs.append(float(row[1]))

with open('../radix_sort/csv files/time_data_rs_ran.csv', 'r') as f:
    reader = csv.reader(f, delimiter='\t') # as headers in a .csv file are separated by tabs
    next(reader) # skip the header row
    for row in reader:
