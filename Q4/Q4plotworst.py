...
with open('../merge_sort/csv files/time_data_ms_dec.csv', 'r') as f:
    reader = csv.reader(f, delimiter='\t') # as headers in a .csv file are separated by tabs
    next(reader) # skip the header row
    for row in reader:
        times_ms.append(float(row[1]))

with open('../heap_sort/csv files/time_data_hs_dec.csv', 'r') as f:
    reader = csv.reader(f, delimiter='\t') # as headers in a .csv file are separated by tabs
    next(reader) # skip the header row
    for row in reader:
        times_hs.append(float(row[1]))

with open('../radix_sort/csv files/time_data_rs_dec.csv', 'r') as f:
    reader = csv.reader(f, delimiter='\t') # as headers in a .csv file are separated by tabs
    next(reader) # skip the header row
    for row in reader:
...
with open('../quick_sort/csv files/time_data_qs_dec.csv', 'r') as f:
    reader = csv.reader(f, delimiter='\t') # as headers in a .csv file are separated by tabs
    next(reader) # skip the header row
    for row in reader:
        if row[1] == '1':
